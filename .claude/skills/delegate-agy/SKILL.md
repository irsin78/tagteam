...
scaffolding/experimental tests (images: the codex launcher `-i` row,
never agy). This skill is the HOW. Use it only when a pure-write task fits the tool
...
- Availability (exit 2 `AGY_UNAVAILABLE`): missing binary or python,
  ...
  nothing) → the orchestrator falls back to claude-implementer.
...
- 0 = DONE (agy SUCCESS, nothing denied, all `-x` outputs proven
  ...
  `FAILED(unauthorized-commit)`) / BLOCKED — read the report. 2 = AGY_UNAVAILABLE → claude-implementer fallback.
